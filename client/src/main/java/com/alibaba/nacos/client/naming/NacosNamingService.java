...
public class NacosNamingService implements NamingService {
    ...
    /**
    * nacos为了方便用户调用，封装的有starter，该starter在初始化的时候会初始化注册实例所需要的bean以及属性
     * 这样才能实现对nacos客户端即nacos-client的调用。
     * 那么nacos的starter是如何实现的呢？其实很简单，通过配置文件进行所需的bean组件注入，然后增加监听器，让spring容器在发布
     * WebServerInitializedEvent事件时触发自己的监听逻辑，监听逻辑就是根据配置文件组装instance实例，将instance实例通过
     * NamingService接口的namingService.registerInstance(serviceId, group, instance);方法进行实例注册到命名空间下面的
     * 不同的组不同的服务上
     * nacos不整合springcloud依然可以使用，但是如果你整合springcloud你就要按照springboot的规范，也整个springbootstarter，
     * 当你整starter的时候其实实现的方式是相同的
    */
    @Override
    public void registerInstance(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        if (instance.isEphemeral()) {
            // 建立心跳 5秒执行一次   DEFAULT_HEART_BEAT_INTERVAL = TimeUnit.SECONDS.toMillis(5);
