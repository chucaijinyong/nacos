...
//        key：com.alibaba.nacos.naming.domains.meta.namespaceId##serviceName  value:service
    ...
    public void putService(Service service) {
        if (!serviceMap.containsKey(service.getNamespaceId())) {
            synchronized (putServiceLock) {
                if (!serviceMap.containsKey(service.getNamespaceId())) {
                    // 创建外层map 即Map(namespace, Map(String, String))
                    serviceMap.put(service.getNamespaceId(), new ConcurrentSkipListMap<>());
                }
            }
        }
//        Map(namespace, Map(group::serviceName, Service)).
        // 创建内层map即Map(group::serviceName, Service)
        serviceMap.get(service.getNamespaceId()).putIfAbsent(service.getName(), service);
    }

    private void putServiceAndInit(Service service) throws NacosException {
        // 服务如果不存在就创建   Map(namespace, Map(group::serviceName, Service)).   此时service已经构造好，但是实例还没有放进去
        putService(service);
